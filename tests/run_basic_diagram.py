...
def run_basic_diagram(cf_name_, tau_, is_jam_, cf_param_, car_length_=5., start=0.01, end=1., step=0.02, plot=True):
    diag = BasicDiagram(1000, car_length_, 0, False, cf_mode=cf_name_, cf_param=cf_param_)
    diag.run(start, end, step, resume=False, file_name="result_" + cf_name_ + ("_jam" if is_jam_ else ""),
             dt=tau_, jam=is_jam_)
...
if __name__ == '__main__':
    ...
    tau = 1
    is_jam = False
    cf_param = {"lambda": 0.8, "original_acc": False, "v_safe_dispersed": True, "tau": tau}
    car_length = 7.5
