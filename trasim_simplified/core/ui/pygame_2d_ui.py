...
from trasim_simplified.core.constant import LaneMarkingColor, LaneMarkingType, LaneType, LaneChange
...
class Util(object):
    ...
    @staticmethod
    def get_bounding_box(actor: Vehicle):
        """Gets the bounding box corners of an actor in world space"""
        bb = actor.trigger_volume.extent
        corners = [Location(x=-bb.x, y=-bb.y),
                   Location(x=bb.x, y=-bb.y),
                   Location(x=bb.x, y=bb.y),
                   Location(x=-bb.x, y=bb.y),
                   Location(x=-bb.x, y=-bb.y)]
...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def lane_marking_color_to_tango(lane_marking_color):
            ...
            if lane_marking_color == LaneMarkingColor.White:
                tango_color = COLOR_ALUMINIUM_2

            elif lane_marking_color == LaneMarkingColor.Blue:
                tango_color = COLOR_SKY_BLUE_0

            elif lane_marking_color == LaneMarkingColor.Green:
                tango_color = COLOR_CHAMELEON_0

            elif lane_marking_color == LaneMarkingColor.Red:
                tango_color = COLOR_SCARLET_RED_0

            elif lane_marking_color == LaneMarkingColor.Yellow:
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            ...
            if lane_marking_type == LaneMarkingType.Broken or (lane_marking_type == LaneMarkingType.Solid):
                ...
                if lane_marking_type == LaneMarkingType.SolidBroken:
                    return [(LaneMarkingType.Broken, lane_marking_color, marking_1),
                            (LaneMarkingType.Solid, lane_marking_color, marking_2)]
                elif lane_marking_type == LaneMarkingType.BrokenSolid:
                    return [(LaneMarkingType.Solid, lane_marking_color, marking_1),
                            (LaneMarkingType.Broken, lane_marking_color, marking_2)]
                elif lane_marking_type == LaneMarkingType.BrokenBroken:
                    return [(LaneMarkingType.Broken, lane_marking_color, marking_1),
                            (LaneMarkingType.Broken, lane_marking_color, marking_2)]
                elif lane_marking_type == LaneMarkingType.SolidSolid:
                    return [(LaneMarkingType.Solid, lane_marking_color, marking_1),
                            (LaneMarkingType.Solid, lane_marking_color, marking_2)]

            return [(LaneMarkingType.NONE, LaneMarkingColor.Other, [])]
        ...
        def draw_lane_marking_single_side(surface, waypoints, sign):
            ...
            marking_type = LaneMarkingType.NONE
            previous_marking_type = LaneMarkingType.NONE

            marking_color = LaneMarkingColor.Other
            previous_marking_color = LaneMarkingColor.Other

            markings_list = []
            temp_waypoints = []
            current_lane_marking = LaneMarkingType.NONE
            ...
            for markings in markings_list:
                if markings[0] == LaneMarkingType.Solid:
                    draw_solid_line(surface, markings[1], False, markings[2], 2)
                elif markings[0] == LaneMarkingType.Broken:
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoint in topology:
                ...
                for w in waypoints:
                    ...
                    l = w.get_left_lane()
                    while l and l.lane_type != LaneType.Driving:

                        if l.lane_type == LaneType.Shoulder:
                            shoulder[0].append(l)
                    ...
                    r = w.get_right_lane()
                    while r and r.lane_type != LaneType.Driving:

                        if r.lane_type == LaneType.Shoulder:
                            shoulder[1].append(r)
        ...
        if self.show_connections:
            ...
            for wp in carla_map.generate_waypoints(dist):
                ...
                if wp.lane_change & LaneChange.Right:
                    r = wp.get_right_lane()
                    if r and r.lane_type == LaneType.Driving:
                        pygame.draw.line(map_surface, col, to_pixel(wp), to_pixel(r), 2)
                if wp.lane_change & LaneChange.Left:
                    l = wp.get_left_lane()
                    if l and l.lane_type == LaneType.Driving:
...
class World(object):
    ...
    def start(self, hud, input_control):
        ...
        self._input.control = VehicleControl()
...
class InputControl(object):
    ...
    def _parse_events(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif isinstance(self.control, VehicleControl):
                    if event.key == K_q:
    ...
    def parse_input(self, clock):
        ...
        if not self._autopilot_enabled:
            if isinstance(self.control, VehicleControl):
