...
class DataProcessor:
    ...
    def aggregate(df: pd.DataFrame, lane_id: int, lane_length: float):
        ...
        avg_acc, avg_speed, avg_gap, avg_dv, avg_dhw, avg_thw = \
            np.mean(group[C_Info.a]), np.mean(group[C_Info.v]), \
            np.mean(group[C_Info.gap]), np.mean(group[C_Info.dv]),\
            np.mean(group[C_Info.dhw]), np.mean(group[C_Info.thw])
        # TODO：标准差这样做有待商榷
        group = df.groupby(by=[C_Info.id]).std()
        std_acc, std_speed, std_gap, std_dv, std_dhw, std_thw = \
            np.std(group[C_Info.a]), np.std(group[C_Info.v]), \
            np.std(group[C_Info.gap]), np.std(group[C_Info.dv]), \
            np.std(group[C_Info.dhw]), np.std(group[C_Info.thw])
        ...
        avg_k_by_car_num_lane_length = len(df) / (lane_length * (df[C_Info.step].max() - df[C_Info.step].min())) * 1000
        avg_q_by_v_k = avg_speed * 3.6 * avg_k_by_car_num_lane_length
    ...
    @staticmethod
    def aggregate_as_detect_loop(df: pd.DataFrame, lane_id: int, lane_length: float,
                                 pos: float, width: float,
                                 dt: float, d_step: int, step_range: Sequence[int, int] = None):
        """
        以传感线圈的方式检测交通参数（HCM的平均速度定义）

        :param df: 车辆的基础数据
        :param lane_id: 车道ID
        :param lane_length: 道路长度 [m]
        ...
        for time_start, time_end in time_sections_zip:
            ...
            for id_ in df[C_Info.id].unique():
                ...
                single_data = single_data[(single_data[C_Info.time] >= time_start) &
                                          (single_data[C_Info.time] < time_end)]
                single_data = single_data.sort_values(by=[C_Info.step]).reset_index(drop=True)

                for time, speed, pos in DataProcessor.data_shear(
                        single_data[C_Info.v].to_numpy(), single_data[C_Info.x].to_numpy(),
                        single_data[C_Info.time].to_numpy(), single_data[C_Info.step].to_numpy(),
                        shear_pos=False, shear_step=True):
    ...
    @staticmethod
    def data_shear(temp_=None, pos_=None, time_=None, step_=None, shear_pos=True, shear_step=True):
        return_index = list(np.where(np.diff(np.array(pos_)) < 0)[0]) if shear_pos else []
        return_index_2 = list(np.where(np.diff(np.array(step_)) != 1)[0]) if shear_step else []
