...
class LcScenario:
    def __init__(
        ...
        self.name_2_id: dict[str, int] = {}
        self.id_2_name: dict[int, str] = {}
        ...
        self.hv_cf_name = hv_cf
        self.av_cf_name = av_cf

        self.mode = None
        self.surr_class = None

        self._load_road(osm_path)
        self._clean_traj()

    def _load_road(self, osm_path):
        ...
        self.road: Road = make_road_from_osm(osm_path, map_config, self.weaving_offset)
    ...
    def _clean_traj(self):
        ...
        for i, (traj, name) in enumerate(zip(traj_list, traj_name)):
            ...
            traj[C_Info.xFrontGlobal] = (
                    traj[C_Info.xCenterGlobal].copy() + traj["length"] / 2 * np.cos(traj["heading"])
            )
            traj[C_Info.yFrontGlobal] = (
                    traj[C_Info.yCenterGlobal].copy() + traj["length"] / 2 * np.sin(traj["heading"])
            )
    ...
    def _set_sce_mode(self):
        if self.surr_class.tp_type == V_CLASS.GAME_AV and self.surr_class.tr_type == V_CLASS.GAME_AV:
            self.mode = ScenarioMode.INTERACTION_TR_AV_TP_AV
        elif self.surr_class.tp_type == V_CLASS.GAME_AV and self.surr_class.tr_type == V_CLASS.GAME_HV:
            self.mode = ScenarioMode.INTERACTION_TR_HV_TP_AV
        elif self.surr_class.tp_type == V_CLASS.GAME_HV and self.surr_class.tr_type == V_CLASS.GAME_AV:
            self.mode = ScenarioMode.INTERACTION_TR_AV_TP_HV
        elif self.surr_class.tp_type == V_CLASS.GAME_HV and self.surr_class.tr_type == V_CLASS.GAME_HV:
            self.mode = ScenarioMode.INTERACTION_TR_HV_TP_HV
        else:
            raise ValueError(f"Unknown surr_class: {self.surr_class}")

    def load_vehicle(self, surr_class: SurrClass):
        self.road.reset()
        self._set_record_info()
    ...
    def run(
            ...
            cf_params: dict[int, dict] | dict[str, float] = None,
            car_params: dict[int, dict] | dict[str, float] = None,
            has_ui=True,
            save_res=True,
            save_file_path: str = None
    ):
        self.road.reset()
        self.load_vehicle(surr_class)

        ev = self.surr_ids[self.name_2_id["EV"]][0]
        ...
        for vid, (veh, traj, name) in self.surr_ids.items():
            if isinstance(veh, Game_O_Vehicle):
                ...
                if ev.route_type == RouteType.merge and veh.route_type == RouteType.diverge:
                    pass
                elif ev.route_type == RouteType.diverge and veh.route_type == RouteType.merge:
                    pass
                else:
                    veh.no_lc = True
        ...
        merged_df = self.get_res(save_res, save_file_path)
        return merged_df
    ...
    def get_res(self, save_res=True, save_file_path: str = None):
    ...
    def _load_traj(self, merged_df=None, save_file_path: str = None):
        if merged_df is None:
            # 读取合并后的数据
            merged_df = load_from_pickle(save_file_path)
        traj_s = []
        traj_name = []
    ...
    def plot_scenario(
            self,
            highlight=None,
            res_df_path: str = None,
            fig_name: str = "scenario",
            base_path: str = None
    ):
        if highlight is None:
            highlight = ["EV"]
        merged_df, traj_s, traj_name = self._load_traj(save_file_path=res_df_path)
        plot_scenario_twin(
            traj_s, traj_names=traj_name, road=self.road,
            fig_name=fig_name, highlight=highlight, base_path=base_path
        )

    def indicator_evaluation(self, surr_class, res_df=None, save_file_path: str = None):
        assert res_df is not None or save_file_path is not None

        self.load_vehicle(surr_class)
        df, traj_s, traj_name = self._load_traj(res_df, save_file_path)
    ...
    def opti_ade(self, surr_class, cf_params=None):
        def fitness_func(params):
            print("params: ", params)
            try:
                df = self.run(
                    surr_class=surr_class,
                    car_params={
                        "rho": 0.5,
                        "k_s": 10 ** params[0],
                        "k_c": 10 ** params[1],
                ...
                print(f"Error: {e}")
                return 1000
            ade = self.indicator_evaluation(surr_class, df)
            return ade
        ...
        res_dict = {}
        for vid, (_, _, _) in self.surr_ids.items():
            res_dict[vid] = {}
            for i, (param, value) in enumerate(zip(result.space, result.x)):
                res_dict[vid].update({param.name: 10 ** value})
        print(res_dict)

        return res_dict

    def calibrate_cf(self, surr_class):
        self.load_vehicle(surr_class)
        cf_param_dict_s = {}
        for vid, (car, traj, name) in self.surr_ids.items():
