...
def simulation(cf_func, init_v: float, init_x: float, obs_lv, obs_lx, cf_param, dt,
               leaderL: float | typing.Iterable, update_method="Euler") -> tuple[list, list, list]:
    """
    ...
    if isinstance(leaderL, float | int):
...
def customize_sim(leader_schedule: list[tuple[float, int]], initial_states: list[tuple[float, float, float]],
    ...
    x_list, v_list, a_list = generate_traj(leader_schedule, initial_states[0], dt)
    current_x = x_list[0]
    x_lists, v_lists, a_lists = [x_list], [v_list], [a_list]
    for (gap, v, a), leader_length, cf_funcs, cf_param in zip(initial_states[1:], length_s[:-1], cf_funcs, cf_params):
        current_x -= (gap + leader_length)
        sim_pos, sim_speed, sim_acc = simulation(cf_funcs, init_x=current_x, init_v=v, obs_lv=v_lists[-1],
    ...
    return x_lists, v_lists, a_lists
...
def data_to_df(x_lists, v_lists, a_lists, dt):
