...
def plot_scenario_twin(
        traj_s, traj_names, road: 'Road', fig_name,
        mode=ScenarioMode.NO_INTERACTION, highlight=None,
        ori_plot=True, base_path=None
):
    ...
    ax.set_ylabel("Y (m)")
    fig.savefig(
        fr"{base_path}\{fig_name}_2d_traj.tif",
        dpi=300, pil_kwargs={"compression": "tiff_lzw"})

    plot_ttc(traj_s, traj_names, fig_name, base_path, ori_plot)
    plot_xy_info(traj_s, traj_names, fig_name, base_path, ori_plot, mode=mode, surr_name=traj_names)


def plot_ttc(traj_s, traj_names, fig_name, base_path, ori_plot=True):
    colors = sns.color_palette('tab20', n_colors=len(traj_s))
    fig, ax = get_fig_ax(scale=1)
    # 绘制TTC曲线
    for i, (traj_, name) in enumerate(zip(traj_s, traj_names)):
    ...
    ax.set_ylabel("TTC (s)")
    fig.savefig(
        fr"{base_path}\{fig_name}_TTC.tif",
        dpi=300, pil_kwargs={"compression": "tiff_lzw"}
    )


def plot_xy_info(traj_s, traj_names, fig_name, base_path, ori_plot=True, mode=None, surr_name=None):
    ...
    def plot_y_info(info_name, func, addi, y_name, suffix):
        ...
        fig.savefig(
            fr"{base_path}\{fig_name}_{info_name}_{suffix}.tif",
            dpi=300, pil_kwargs={"compression": "tiff_lzw"}
        )
...
def plot_stra(stra_dict, max_step, fig_name, type_="TR"):
    # 绘制激进度估计与TR策略变化曲线
    tr_id_s = []
    tr_stra_s = []
    tr_rho_s = []
    if stra_dict is not None:
        ...
        fig.savefig(
            fr"E:\BaiduSyncdisk\car-following-model\tests\thesis\fig\{fig_name}_{type_}_stra.png",
